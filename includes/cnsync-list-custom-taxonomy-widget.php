...
add_action('widgets_init', 'init_cnsync_taxonomy');
function init_cnsync_taxonomy()
{
	register_sidebar(array(
		'name' => 'Memberships Sidebar',
		'id' => 'memberships_sidebar',
	...
	return register_widget('cnsync_taxonomy');
}
class cnsync_taxonomy extends WP_Widget
{
	/** constructor */
	function __construct()
	{
		parent::__construct('cnsync_taxonomy', $name = 'List Product Catgories');
	}
	/**
	 * This is the Widget
	 **/
...
?><script>
			...
			function lctwContract(id) {
				...
				jQuery('.lctw-expand-options').show(500);
					...
					if ($instance) {
						...
						$childof  = '';
						$this_taxonomy = 'cloudnet_category'; //this will display the category taxonomy, which is used for normal, built-in posts
						$hierarchical = true;
						$showcount = true;
						$empty = false;
						$dropdown = false;
		...
		<input type="hidden" name="<?php echo $this->get_field_name('taxonomy'); ?>" id="<?php echo $this->get_field_id('taxonomy'); ?>" value="cloudnet_category" />
